...
#  1. Open RStudio and set your working directory to the folder where you want
#     the output files saved. You can do this in two ways:
#       (a) Via the RStudio menu: Session > Set Working Directory > Choose Directory
#       (b) By running this line first (replace the path with your own folder):
#               setwd("C:/your/folder/here")
#  2. Run this script. All output files will be saved automatically to a
#     subfolder called "CITC_AIOSR_Output" inside your working directory.
#  ────────────────────────────────────────────────────────────────────────────
################################################################################
...
seed_base    <- 2026L
R_sim        <- 500L          # simulation replications (use 100 for a quick test)
target_alpha <- 0.80          # desired internal consistency

# Output directory: automatically created inside your current working directory.
# To change where files are saved, run setwd("your/path") before running this script.
out_dir <- file.path(getwd(), "CITC_AIOSR_Output")
if (!dir.exists(out_dir)) {
  dir.create(out_dir, recursive = TRUE)
  message(sprintf("Output folder created: %s", normalizePath(out_dir)))
} else {
  message(sprintf("Output folder found: %s", normalizePath(out_dir)))
}
...
#    Column headers match the Bias/RMSE tables:
#    r-raw | Guilford | Cureton | Henrysson | CITC | AIOSR
################################
...
message("\n=== ALL DONE ===")
message(sprintf("All output files are in: %s", normalizePath(out_dir)))

################################################################################
# COLUMN KEY (matches Results and Discussion in manuscript):
#
